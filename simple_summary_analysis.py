...
for i in range(len(name_genes_grna_unique)):
  for j in range(indel_prop_matrix.shape[0]):
    if name_indel_type_unique[j].find('I') != -1:
      prop_insertions_gene_grna[i] += np.sum(indel_prop_matrix[j][3*i:3*i+3], dtype = float)
    if name_indel_type_unique[j].find('D') != -1:
      prop_deletions_gene_grna[i] += np.sum(indel_prop_matrix[j][3*i:3*i+3], dtype = float)
...
for i in range(len(unique_pam)):
  print unique_pam[i]
  print unique_pam_ins[i]/len(unique_pam)
  print unique_pam_del[i]/len(unique_pam)

for i in range(len(unique_sequence)):
  print unique_sequence[i]
  print unique_seq_ins[i]/len(unique_sequence)
  print unique_seq_del[i]/len(unique_sequence)